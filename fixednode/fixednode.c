...
static uint8_t local_sensor_value = 0x00;
...
PROCESS(homeauto_fixednode_process, "Fixed Node");
PROCESS(homeauto_fixednode_uc_process, "Fixed Node Unicast");
...
static void unicast_recv(struct unicast_conn *c, const linkaddr_t *from)
{
   ...
   * If the user instructs the light to be on then switch it on.
...
AUTOSTART_PROCESSES(&homeauto_fixednode_process, &homeauto_fixednode_uc_process);

PROCESS_THREAD(homeauto_fixednode_process, ev, data)
{
...
PROCESS_THREAD(homeauto_fixednode_uc_process, ev, data)
{
	...
	while (1) {
		...
		*/
		if (ev == button_press_duration_exceeded) {
			...
			local_sensor_value = (local_sensor_value + 1) % 2;
		}
...
{	
	...
	case TX_PACKET_GEN_SENSOR_VAL:
		...
		/* Fixed node priority = 0x01 */
		tx_packet_uc.priority = 0x01;
